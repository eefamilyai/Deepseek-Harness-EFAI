...
- **Pins and a fold on Kiln** (`llm-kiln`). `compact-checkpoint` and `session-recovery` turns are pinned, so a re-prime never clips them. A summary request too large for one capped prompt on `ds_direct` is folded in parts, each carrying the running summary inside the `<compacted-summary>` tags upstream's summarizer already merges.
