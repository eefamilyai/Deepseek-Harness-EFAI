  ...
  ctx.inject(['systemPrompt'], (scope: Context) => {
    ...
    scope.systemPrompt.context({
      ...
      text: (context) => {
        ...
        const ledger = ledgerOf(session)
        const id = ledger.compaction?.id
        // The same path the pre-step writes the record to.
        const recordPath = id === undefined
          ? undefined
          : join(sessionDir(root, session.header.cwd, session.id), compactionRecordFilename(id, session.id))
        return renderFocus(ledger, recordPath)
      },
    })
  })
}
