 ...
 * own todo file keeps the goal in view. It also names the latest compaction's
 * record file, so the full handoff and checkpoint stay one read away for as
 * long as the session runs, without their text riding every request.
 * @param ledger - the folded ledger.
 * @param recordPath - the latest compaction's record file, when known.
 * @returns the focus text, or `''` before any compaction.
 */
export function renderFocus(ledger: Ledger, recordPath?: string): string {
  ...
  const parts = [
    ...
    recordPath === undefined ? '' : `full record: ${recordPath}`,
