...
async function mount(options: { kernelEnabled?: boolean } = {}): Promise<Context> {
  const ctx = new Context()
  await ctx.plugin(MemorySettings, { doc: { kernel: { enabled: options.kernelEnabled ?? true } } })
...
describe('a change lands when the model stops generating', () => {
  ...
  it('publishes a per-tool default for every mounted tool', async () => {
    ...
    expect(Object.keys(base.tools).sort()).toEqual([
      ...
      'send_message', 'subagent', 'subagent_fork',
    ])
