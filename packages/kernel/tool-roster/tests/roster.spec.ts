...
/** Mount the roster over real settings, tools, and prompt services. */
async function mount(options: { kernelEnabled?: boolean; rlmEnabled?: boolean } = {}): Promise<Context> {
  const ctx = new Context()
  await ctx.plugin(MemorySettings, {
    doc: {
      kernel: { enabled: options.kernelEnabled ?? true },
      rlm: { enabled: options.rlmEnabled ?? false },
    },
  })
  // The two switch namespaces as their owners publish them (`kernel-mode` and
  // `rlm-mode`): the roster reads them through the service, so an unregistered
  // namespace would read as its fallback rather than as the document says.
  ctx.settings.register(KERNEL_SETTINGS_NAMESPACE, z.object({
    enabled: z.boolean().default(true),
  }), { base: { enabled: options.kernelEnabled ?? true }, applies: 'live' })
  ctx.settings.register(RLM_SETTINGS_NAMESPACE, z.object({
    enabled: z.boolean().default(false),
  }), { base: { enabled: options.rlmEnabled ?? false }, applies: 'live' })
  ...
  await ctx.plugin({ name: 'tool-roster', apply, Config }, {})
  return ctx
}
...
describe('a hidden tool loses its schema and its guidance together', () => {
  ...
  it('drops the hidden tool and its section from the assembled prompt', async () => {
    ...
    await ctx.settings.update(TOOLS_SETTINGS_NAMESPACE, { tools: { bash: false } })
...
describe('visibility is not enforcement', () => {
  ...
  it('denies every non-kernel tool when the category switch is off', async () => {
    const ctx = await mount()
    await ctx.settings.update(TOOLS_SETTINGS_NAMESPACE, { enabled: false })
    await ctx.parallel('agent/turn-stopping', { agent: subject, turn: 1, signal })
...
describe('the acting surface follows the rlm switch', () => {
  it('withdraws the kernel tool and offers the engine when RLM is on at boot', async () => {
    const ctx = await mount({ rlmEnabled: true })
  ...
  it('swaps the two surfaces at the turn boundary when the switch is flipped', async () => {
    ...
    await ctx.settings.update(RLM_SETTINGS_NAMESPACE, { enabled: true })
    await ctx.parallel('agent/turn-stopping', { agent: subject, turn: 1, signal })
...
describe('a change lands when the model stops generating', () => {
  ...
  it('starts under what the document already said, without waiting for a boundary', async () => {
    ...
    const ctx = await mount({ kernelEnabled: false })
    expect((await assemble(ctx)).tools).not.toContain('kernel')
    expect(await run(ctx, 'kernel')).toContain('switched off in settings')
    await ctx.fiber.dispose()
  })

  it('publishes a per-tool default for every mounted tool', async () => {
    const ctx = await mount()
    const descriptors = ctx.settings.describe()
      .filter(entry => entry.ns === TOOLS_SETTINGS_NAMESPACE)
    expect(descriptors).toHaveLength(1)
    const base = descriptors[0]!.base as { tools: Record<string, boolean> }
    // The names arrive from the registry, which is what lets a configuration
    // surface render one switch per tool without hardcoding a list.
    // Globally registered tools arrive from the registry; the subagent family
    // installs its tools into each agent's own scope, so those names are seeded
    // explicitly and must appear here too — the operator expects a switch for
    // every tool the model can call, wherever the plugin put it.
    expect(Object.keys(base.tools).sort()).toEqual([
      'bash', 'interrupt_agent', 'kernel', 'list_agents', 'read',
      'rlm', 'send_message', 'subagent', 'subagent_fork',
    ])
    expect(Object.values(base.tools).every(value => value === true)).toBe(true)
    expect(descriptors[0]!.applies).toBe('live')
    await ctx.fiber.dispose()
  })
})
