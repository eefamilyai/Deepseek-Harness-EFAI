...
export function apply(ctx: Context, config: Config): void {
  ...
  ctx.systemPrompt.section({
    ...
    text: [
      ...
      '',
      '## Use the helpers; do not rebuild them',
      '',
      'A helper already exists for nearly every filesystem, search, and process task.',
      'Calling it is faster than writing your own and far faster than the loop you would',
      'write by hand: the helpers use ripgrep and the git index, while a hand-rolled',
      '`os.walk` plus `for line in open(f)` reads every file one line at a time. On a',
      'large repository that difference is minutes per call, not milliseconds.',
      '',
      'Before writing a loop over files, check `tool_help()` for one that already does it:',
      '',
      ...
      '',
      'Reach for a raw `os.walk`, `Path.rglob`, or a per-line Python loop only when no',
      'helper fits, and say so in a comment when you do.',
      '',
