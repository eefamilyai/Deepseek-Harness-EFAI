...
Sixteen rows, grouped by subject: the identity opener (`efai-identity`), the LLM layer (`llm-dsml`, `llm-kiln`), post-compaction grounding (`session-recovery-context`), the acting surface (`kernel-mode`, `kernel`, `kernel-python`, `tool-kernel`, `kernel-rlm-context`, `rlm-mode`, `rlm`), durable memory (`agent-memory-mode`), the session-info command, upstream's `tool-str-replace-editor`, the self-hosted browser (`web-browser`), and the runtime roster (`tool-roster`) last.
