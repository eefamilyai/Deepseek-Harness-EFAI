...
Preferences are durable. The Host half declares the preferences as live config fields of its row and the browser half reads and writes them through that row's config form, with a process-local fallback when the form is unavailable.
...
| [`src/effects-settings.ts`](src/effects-settings.ts) | The preference field names, defaults, and value type, shared by the Host and browser halves |
...
| [`src/client/settings-store.ts`](src/client/settings-store.ts) | Mirrors the row's config form into browser state, with a process-local fallback |
...
- **Settings fall back silently** — when the `ui-effects` row's config form is unavailable the store uses process-local values, so a change made in that state is not durable.
