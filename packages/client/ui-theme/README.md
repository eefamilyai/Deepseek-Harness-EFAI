...
`design-platform.css` also declares the `--dsw-alias-flow-*` accent group: one hue per compact flow-row family — `think`, `search`, `read`, `mutate` (write and edit), `shell`, and `code` — that ui-chat's think row and ui-tool's tool rows apply to the leading glyph, the title, the hover chevron, and the separator dot. The group sits apart from `--dsw-alias-state-*`, which means run state, and avoids the error and warn ramps so an accent never reads as a failed or interrupted row. Each theme resolves its own static step, clearing 4.5:1 against that theme's `--dsw-alias-bg-base`. Unclassified generic rows set no accent and keep the neutral label tones.
