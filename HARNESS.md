...
| Package | What it does |
|---|---|
| `llm/llm-kiln` | registers every Kiln provider as a harness route named `<prefix><id>` (default `kiln-`): Anthropic, OpenAI, Gemini, OpenRouter, DeepSeek paid, Groq, xAI, Mistral, Together, Fireworks, Perplexity, Cerebras, NVIDIA, Ollama, LM Studio, and `ds_direct` (the free DeepSeek web session), one route per pooled login. Keys are never configuration — each preset names an environment variable. A compaction request gets a summarizer system statement instead of the tool protocol, whichever compaction engine sent it; on `ds_direct`, one too large for a single capped prompt can be folded across at most `compactionFoldParts` calls (default 1: no folding, because each fold call is a fresh web chat and an unbounded fold leaves a long session's compaction pending for minutes). The compaction checkpoint and the recovery handoff are pinned, so a re-primed web chat never clips them. |
