...
flowchart LR
  ...
  pkg_kernel["kernel"]
  svc_kernel["ctx.kernel<br/>Persistent code-execution kernel"]
  ...
  pkg_kernel --> svc_kernel
  pkg_kernel_python --> svc_kernel
  pkg_llm --> svc_llm
  pkg_llm_deepseek --> svc_llm
  pkg_llm_kiln --> svc_llm
...
| `ctx.llm` | `seam` | [`llm`](../packages/llm/llm) | [`llm-deepseek`](../packages/llm/llm-deepseek), [`llm-kiln`](../packages/llm/llm-kiln), [`llm-pi-ai`](../packages/llm/llm-pi-ai), [`llm-replay`](../packages/test-support/llm-replay) | [`agent-loop`](../packages/core/agent-loop), [`compaction-basic`](../packages/compaction/compaction-basic) | - | 适配器注册提供方实现；agent loop（智能体循环）与压缩功能调用提供方无关的流服务。 |
| `ctx.kernel` | `seam` | [`kernel`](../packages/kernel/kernel) | [`kernel-python`](../packages/kernel/kernel-python) | [`tool-kernel`](../packages/kernel/tool-kernel) | - | 一个后端拥有跨调用存续的单一命名空间；工具在其中运行一个 cell 并返回捕获的输出。 |
